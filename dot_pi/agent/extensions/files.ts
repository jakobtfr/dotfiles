...
> => {
  const actions: SelectItem[] = [
    ...(options.canDiff ? [{ value: "diff", label: "Diff in Nvim" }] : []),
...
const openDiff = async (
  ...
  await ctx.ui.custom<null>((tui, theme, _kb, done) => {
    const status = new Text(
      theme.fg("dim", `Opening nvim diff for ${target.displayPath}...`),
    );

    queueMicrotask(() => {
      try {
        tui.stop();
        // Use fugitive's :Gvdiffsplit so we don't need a manual HEAD-blob tmpfile.
        // Falls back to vanilla `nvim -d` if fugitive errors (rare given user config).
        spawnSync(
          "nvim",
          ["-c", `Gvdiffsplit! HEAD -- ${relativePath}`, target.resolvedPath],
