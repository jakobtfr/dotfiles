...
- Dropped the custom rm shim; rely on the system `trash` command for recoverable deletes.

## 2025-12-17 — Remove Runner; Keep Guardrails
- Removed the runner wrapper now that modern Codex sessions handle long-running/background work directly.
- Dropped the `find -delete` interception.
...
- Added lock cleanup, tightened path ignores, and refreshed misc. helper utilities (e.g., `toArray`) to reduce drift across repos.
